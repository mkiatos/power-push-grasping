...
class HeightmapDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        depth_heightmap = np.pad(heightmap, padding_width, 'constant', constant_values=-0.01)
        ...
        angle = (action['theta'] + (2 * np.pi)) % (2 * np.pi)
        rot_id = round(angle / (2 * np.pi / 16))

        # Create label map
        # opening_id = np.argwhere(self.widths == action['opening'])[0, 0]
        opening_id = 0
        action_area = np.zeros((len(self.widths), heightmap.shape[0], heightmap.shape[1]))
        action_area[opening_id, action['pxl'][1], action['pxl'][0]] = 1.0
        label = np.zeros((len(self.widths), padded_shape[0], padded_shape[1]))
        label[opening_id,
              padding_width:padded_shape[0] - padding_width,
              padding_width:padded_shape[1] - padding_width] = action_area[opening_id]

        return depth_heightmap, rot_id, label
...
class ApertureDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        p1 = action['pxl'] + padding_width
        theta = -((action['theta'] + (2 * np.pi)) % (2 * np.pi))
        ...
        if self.plot:
            p2 = np.array([0, 0])
            p2[0] = p1[0] + 20 * np.cos(action['theta'])
            p2[1] = p1[1] - 20 * np.sin(action['theta'])
